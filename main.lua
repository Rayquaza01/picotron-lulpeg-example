--[[pod_format="raw",created="2024-11-13 00:39:47",modified="2025-08-24 13:00:03",revision=205,xstickers={}]]
include("require.lua")

-- https://gist.github.com/golgote/0b5dd012872e3f0623a98d1e1783ebe7
...
local bfunc = {
    ...
    ["%"] = function(a,b) return a % b end,
    ["^"] = function(a,b) return a ^ b end,
    atan2 = math.atan2, max = math.max, min = math.min, pow = m_pow
}
...
function _init()
	dprint(matheval.parse("nan"), 0/0)
	dprint(matheval.parse("inf"), math.huge)
	dprint(matheval.parse("isinf(1e1000)"), ufunc.isinf(1e1000))
	dprint(matheval.parse("isnan(0/0)"), ufunc.isnan(0/0))
	dprint(matheval.parse("sgn(-5)"), ufunc.sgn(-5))
	dprint(matheval.parse("6%4"), 6%4)
	dprint(matheval.parse("3*(5-7^8)"), 3*(5-7^8))
	dprint(matheval.parse("5 + 2*3 - 1 + 7 * 8"), 5 + 2*3 - 1 + 7 * 8)
	dprint(matheval.parse("67 + 2 * 3 - 67 + 2/1 - 7"), 67 + 2 * 3 - 67 + 2/1 - 7)
	dprint(matheval.parse("(5*7/5) + (23) - 5 * (98-4)/(6*7-42)"), (5*7/5) + (23) - 5 * (98-4)/(6*7-42))
	dprint(matheval.parse("(2) + (17*2-30) * (5)+2 - (8/2)*4"), (2) + (17*2-30) * (5)+2 - (8/2)*4)
	dprint(matheval.parse("2*3*4/8 -   5/2*4 +  6 + 0/3"), 2*3*4/8 -   5/2*4 +  6 + 0/3)
	dprint(matheval.parse("2*3 - 4*5 + 6/3"), 2*3 - 4*5 + 6/3)
	dprint(matheval.parse("tan(.7438E+0)"), math.tan(.7438E+0))
	dprint(matheval.parse("sin(pi)"), math.sin(math.pi))
	dprint(matheval.parse("pow(2,3)"), 2^3)
	dprint(matheval.parse("min(2,3)"), math.min(2,3))
	dprint(matheval.parse("max(2,3)"), math.max(2,3))
	dprint(matheval.parse("y0_abc_32_def + 2",{ y0_abc_32_def = 1 }), 3)
end